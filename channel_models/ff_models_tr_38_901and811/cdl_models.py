...
Purpose:
Implementation of the CDL channel as defined in ETSI TR_38.901 v17.0.0.
...
class CDL_models(object):
    ...
    def __init__(self, channel_model, los, o2i, atmospheric_absorption, desired_delay_spread, cdl_type, rng,
        ...
        self.cdl_type = cdl_type  # nlos: A, B, C; los: C, D
        # self.rms_delay = rms_delay  #
        self._rng = rng

        self.cp = cdl_ma.cdl_cp[self.cdl_type]
        ...
        self.pcp = cdl_ma.cdl_pcp[self.cdl_type]
        ...
        self.tx_antenna_mode = tx_antenna_mode
    ...
    def step_3_get_cross_polarization_power_ratios(
        ...
        xpr_factor_n_m = np.full((self.n, self.m), xpr_factor)

        return xpr_factor_n_m
    ...
    def step_10_draw_initial_random_phases(self):
        phases = np.random.uniform(-np.pi, np.pi, (4, self.n, self.m))
    ...
    def step_11_nlos_channel_coefficients(self, shuffled_aod_array, shuffled_aoa_array, shuffled_zod_array,
                                          shuffled_zoa_array, xpr_factor_n_m):
        ...
        for u in range(self.u):
            for s in range(self.s):
                ...
                # print("ol_n_fc", ol_n_fc)

                for n in range(self.n):
                    h_sum_m = 0
                    for m in range(self.m):

                        theta_rx = shuffled_zoa_array[n][m]
                        phi_rx = shuffled_aoa_array[n][m]
                        pfc_rx = self.step_11_2_get_polarized_field_component_nlos(self.rx_antenna_mode, theta_rx, phi_rx)
                        # print("nlos pfc_rx", pfc_rx)

                        theta_tx = shuffled_zod_array[n][m]
                        phi_tx = shuffled_aod_array[n][m]
                        pfc_tx = self.step_11_2_get_polarized_field_component_nlos(self.tx_antenna_mode, theta_tx, phi_tx)
                        # print("nlos pfc_tx", pfc_tx)

                        phase_exp_mat = np.array([[np.exp(1j * phases[polarizations['θθ']][n][m]),
                                                   np.sqrt(xpr_factor_n_m[n][m] ** -1) * np.exp(
                                                       1j * phases[polarizations['θϕ']][n][m])],
                                                  [np.sqrt(xpr_factor_n_m[n][m] ** -1) * np.exp(
                                                      1j * phases[polarizations['ϕθ']][n][m]),
                                                   np.exp(1j * phases[polarizations['ϕϕ']][n][m])]])
                        ...
                        r_rx_n_m = self.step_11_1_spherical_unit_vector_arrival(theta_rx, phi_rx)
                        r_tx_n_m = self.step_11_1_spherical_unit_vector_arrival(theta_tx, phi_tx)

                        term1 = np.exp(1j * 2 * np.pi * np.dot(r_rx_n_m.T, d_rx_u) / self.lambda_0)
                        term2 = np.exp(1j * 2 * np.pi * np.dot(r_tx_n_m.T, d_tx_s) / self.lambda_0)
                        term3 = np.exp(1j * 2 * np.pi * np.dot(r_rx_n_m.T, v_vec) * self.t_now / self.lambda_0)
                        result = term1 * term2 * term3

                        h_nlos_u_s_n_m[u][s][n][m] = (np.sqrt(self.c_power[n] / self.m) * (
                                np.dot(pfc_rx.T, phase_exp_mat) * np.dot(pfc_tx, result)))[0]
                        h_sum_m += (np.dot(pfc_rx.T, phase_exp_mat) * np.dot(pfc_tx, result))
                    oxygen_loss = 10 ** (ol_n_fc[n] / 10)
                    # print("nlos h_sum_m", h_sum_m)
                    h_nlos_n = np.sqrt(self.c_power[n] * oxygen_loss / self.m) * h_sum_m
                    h_nlos_u_s_n[u][s][n] = h_nlos_n[0]

        return h_nlos_u_s_n, h_nlos_u_s_n_m

    # In the LOS case, determine the LOS channel coefficient by:
    def step_11_los_channel_coefficients(self, shuffled_aod_array, shuffled_aoa_array, shuffled_zod_array,
                                         shuffled_zoa_array):
        ...
        for u in range(self.u):
            for s in range(self.s):
                ...
                term2 = np.exp(1j * 2 * np.pi * np.dot(r_tx_n_m.T, d_tx_s) / self.lambda_0)
                term3 = np.exp(1j * 2 * np.pi * np.dot(r_rx_n_m.T, v_vec) * self.t_now / self.lambda_0)
                term4 = np.exp(-1j * 2 * np.pi * self.d_3d / self.lambda_0)
                result = term1 * term2 * term3 * term4

                oxygen_loss = 10 ** (ol_n_fc[0] / 10)
                h_sum_m = np.sqrt(oxygen_loss) * ((np.dot(pfc_rx.T, phase_exp_mat) * np.dot(pfc_tx, result)))

                h_los_u_s_1[u][s] = h_sum_m[0]

        return h_los_u_s_1

    # In the NLOS case, the channel impulse response is given by: (When fallowing procedure that consider two strongest clusters (n=1, 2), page 40.)
    # def step_11_nlos_channel_impulse_response_v2(self, shuffled_aod_array, shuffled_aoa_array, shuffled_zod_array,
    #                                           shuffled_zoa_array, xpr_factor_n_m):
    #
    #     dds = self.get_desired_delay_spread()
    #
    #     h_nlos_u_s_n, h_nlos_u_s_n_m = self.step_11_nlos_channel_coefficients(shuffled_aod_array, shuffled_aoa_array,
    #                                                                           shuffled_zod_array, shuffled_zoa_array,
    #                                                                           xpr_factor_n_m)
    #
    ...
    #                     for m in R_i[i]:
    #                         delta_ni = self.delta_function(tau,
    #                                                        dds * self.c_delay_normalized[n] * 1e-9 + delay_offset[i],
    #                                                        dt)
    #
    ...
    #
    #             for n in range(2, self.n):  # for n = 2 to n_size-1 in zero-indexed Python
    #
    ...
    # In the NLOS case, the channel impulse response is given by: (When fallowing procedure that consider that all clusters are treated as
    # "weaker cluster", i.e. no further sub-clusters in delay should be generated, page 74.)
    def step_11_nlos_channel_impulse_response(self, shuffled_aod_array, shuffled_aoa_array, shuffled_zod_array,
                                                 shuffled_zoa_array, xpr_factor_n_m):
        ...
        # Define a time vector for tau
        dt = 1e-9  # Time resolution (1 ns)
        tau_length = 25000  # Length of the time vector
        tau = np.arange(tau_length) * dt  # Example time vector from 0 to 1000 ns
    ...
    def step_11_los_channel_impulse_response(self, shuffled_aod_array, shuffled_aoa_array, shuffled_zod_array,
                                             shuffled_zoa_array, xpr_factor_n_m):
        ...
        h_nlos_u_s_tau = self.step_11_nlos_channel_impulse_response(shuffled_aod_array, shuffled_aoa_array,
                                                                    shuffled_zod_array, shuffled_zoa_array,
                                                                    xpr_factor_n_m)
        #
        ...
        h_los_u_s_1 = self.step_11_los_channel_coefficients(shuffled_aod_array, shuffled_aoa_array, shuffled_zod_array,
                                                            shuffled_zoa_array)
...
def calculate_oxygen_loss(channel_model, los, o2i, atmospheric_absorption, desired_delay_spread, cdl_type,
                                rng, num_rx_ax, num_tx_ax, tx_antenna_mode, rx_antenna_mode,
                                ax_panel_polarization, delta_h, d_2d, d_3d, bearing_angle,
                                down_tilt_angle, h_angle, v_angle, fc, rx_speed, rx_coord, tx_coord, t_now, t_old):
    cdl = CDL_models(channel_model, los, o2i, atmospheric_absorption, desired_delay_spread, cdl_type, rng,
                     num_rx_ax, num_tx_ax, tx_antenna_mode, rx_antenna_mode,
                     ax_panel_polarization, delta_h, d_2d, d_3d, bearing_angle,
                     down_tilt_angle, h_angle, v_angle, fc, rx_speed, rx_coord, tx_coord, t_now, t_old)

    ol_n_fc = cdl.atmospheric_absorption
    return ol_n_fc


def cdl_ff_channel_coefficients(channel_model, los, o2i, atmospheric_absorption, desired_delay_spread, cdl_type,
                                rng, num_rx_ax, num_tx_ax, tx_antenna_mode, rx_antenna_mode,
                                ax_panel_polarization, delta_h, d_2d, d_3d, bearing_angle,
                                down_tilt_angle, h_angle, v_angle, fc, rx_speed, rx_coord, tx_coord, t_now, t_old):
    cdl = CDL_models(channel_model, los, o2i, atmospheric_absorption, desired_delay_spread, cdl_type, rng,
    ...
    xpr_factor = cdl.step_3_get_cross_polarization_power_ratios()
    if los:
        h_los_u_s_1 = cdl.step_11_los_channel_coefficients(shuffled_aod_array, shuffled_aoa_array, shuffled_zod_array,
                                                           shuffled_zoa_array)
        channel_coefficients = h_los_u_s_1
    else:
        h_nlos_u_s_n, h_nlos_u_s_n_m = cdl.step_11_nlos_channel_coefficients(shuffled_aod_array, shuffled_aoa_array,
                                                                             shuffled_zod_array, shuffled_zoa_array,
                                                                             xpr_factor)
...
def cdl_ff_channel_impulse_response(channel_model, los, o2i, atmospheric_absorption, desired_delay_spread, cdl_type,
                                    rng, num_rx_ax, num_tx_ax, tx_antenna_mode, rx_antenna_mode,
                                    ax_panel_polarization, delta_h, d_2d, d_3d, bearing_angle,
                                    down_tilt_angle, h_angle, v_angle, fc, rx_speed, rx_coord, tx_coord, t_now, t_old):
    cdl = CDL_models(channel_model, los, o2i, atmospheric_absorption, desired_delay_spread, cdl_type, rng,
    ...
    if los:
        h_los_u_s_tau = cdl.step_11_los_channel_impulse_response(shuffled_aod_array, shuffled_aoa_array,
                                                                 shuffled_zod_array, shuffled_zoa_array, xpr_factor)
        channel_impulse_response = h_los_u_s_tau
    else:
        h_nlos_u_s_tau = cdl.step_11_nlos_channel_impulse_response(shuffled_aod_array, shuffled_aoa_array,
                                                                   shuffled_zod_array, shuffled_zoa_array, xpr_factor)
        channel_impulse_response = h_nlos_u_s_tau
    return channel_impulse_response


def cdl_ff_channel_gain(channel_model, los, o2i, atmospheric_absorption, desired_delay_spread, cdl_type, rng,
                        num_rx_ax, num_tx_ax, tx_antenna_mode, rx_antenna_mode,
                        ax_panel_polarization, delta_h, d_2d, d_3d, bearing_angle,
                        down_tilt_angle, h_angle, v_angle, fc, rx_speed, rx_coord, tx_coord, t_now, t_old):
    cdl = CDL_models(channel_model, los, o2i, atmospheric_absorption, desired_delay_spread, cdl_type, rng,
    ...
    if los:
        ...
        for u in range(num_rx_ax):
            for s in range(num_tx_ax):
                ...
                # channel_gain_lineal[u][s] = np.abs(np.sum(h_nlos_u_s_tau[u][s])) ** 2
                channel_gain_db[u][s] = 10 * np.log10(channel_gain_lineal[u][s])
...
#     desired_delay_spread = "Very short"  # Nominal, Very long, Long delay, Short, Very short, None
#
#
#     cg = cdl_ff_channel_gain(channel_model, los, o2i, atmospheric_absorption,
#                              desired_delay_spread, cdl_type, rng, num_rx_ax,
